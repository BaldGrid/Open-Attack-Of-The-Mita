...
public class SleepingPrize : MonoBehaviour
{
	...
	private void Update()
	{
		...
		if ((((Input.GetMouseButtonDown(0) || Input.GetKeyDown(KeyCode.E)) && Time.timeScale != 0f) & (base.name == "SleepingPrize")) && Physics.Raycast(Camera.main.ScreenPointToRay(new Vector3(Screen.width / 2, Screen.height / 2, 0f)), out var _) && Vector3.Distance(player.position, base.transform.position) < openingDistance)
		{
