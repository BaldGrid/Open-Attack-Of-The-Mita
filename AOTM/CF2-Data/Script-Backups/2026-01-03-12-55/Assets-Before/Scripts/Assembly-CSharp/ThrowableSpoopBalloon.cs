...
public class ThrowableSpoopBalloon : MonoBehaviour
{
	...
	private void Update()
	{
		if ((Input.GetMouseButtonDown(0) || (Input.GetKeyDown(KeyCode.E) && Time.timeScale != 0f)) && Physics.Raycast(Camera.main.ScreenPointToRay(new Vector3(Screen.width / 2, Screen.height / 2, 0f)), out var hitInfo) && ((hitInfo.transform.tag == "SpoopBalloon") & (Vector3.Distance(player.position, base.transform.position) < 12f) & !thrown))
		{
			thrown = true;
