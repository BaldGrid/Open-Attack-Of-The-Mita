...
public class BossController : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        if (bossSpawn == null)
        {
            bossSpawn = transform.Find("BossSpawn")?.gameObject;
        }
    }
	...
	public IEnumerator WaitForNULL()
	{
		...
		// 直接使用入口的位
        // 直接使用入口的位置
        ns.target = gc.entrances[gc.lastestExit].transform;

        // 修改等待逻辑
        Vector3 targetPos = gc.entrances[gc.lastestExit].transform.position;
        while (Vector3.Distance(ns.gameObject.transform.position, targetPos) > 0.1f)
        {
            yield return null;
        }
    }
