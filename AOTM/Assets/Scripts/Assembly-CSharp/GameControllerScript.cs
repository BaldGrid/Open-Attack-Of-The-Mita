...
public class GameControllerScript : MonoBehaviour
{
	...
	public Animator NotebooksAnim1;
	
	public AudioClip NotebooksSound;

	public AudioClip NotebooksSound1;

	public GameControllerScript()
	{
		itemSelectOffset = new int[5] { -165, -125, -85, -45, -6 };
	}
	...
	private void Update()
	{
		...
		if (!learningActive)
		{
			...
			if (ControlFreak2.CF2Input.GetKeyDown(KeyCode.Y) & gamePaused)
			{
				ExitGame();
			}
			else if (ControlFreak2.CF2Input.GetKeyDown(KeyCode.N) & gamePaused)
			{
				UnpauseGame();
			}
			...
			if (ControlFreak2.CF2Input.GetMouseButtonDown(1) || (ControlFreak2.CF2Input.GetKeyDown(KeyCode.Q) && Time.timeScale != 0f))
			{
				UseItem();
			}
			if (ControlFreak2.CF2Input.GetAxis("Mouse ScrollWheel") > 0f && Time.timeScale != 0f)
			{
				DecreaseItemSelection();
			}
			else if (ControlFreak2.CF2Input.GetAxis("Mouse ScrollWheel") < 0f && Time.timeScale != 0f)
			{
	...
	public void StartFirstBoss()
	{
        ...
        foreach (Material item in matsToModfiy)
        {
            item.SetFloat("_Speed", 20f);
            item.SetFloat("_Amnt", 1f);
        }
        hud.SetActive(value: false);
		fadeing.SetActive(value: true);
	...
	{
		NotebooksAnim.Play("NotebookImageAnimation1", -1); 
		audioDevice.PlayOneShot(NotebooksSound1);
		auds.enabled = true;
		fadeing.SetActive(value: true);
		camera.cullingMask = cullingMask;
		learningActive = false;
	...
	public void CollectItem(int item_ID)
	{
		...
		{
			item[3] = item_ID;
			itemSlot[3].texture = itemTextures[item_ID];
		}
		else if (item[4] == 0)
		{
			item[4] = item_ID;
			itemSlot[4].texture = itemTextures[item_ID];
		}
		else
		{
			item[itemSelected] = item_ID;
