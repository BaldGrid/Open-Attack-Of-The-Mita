...
public class PlaytimeScript : MonoBehaviour
{
	...
	private void Update()
	{
		...
		{
			playCool = 0f;
			animator.SetBool("disappointed", value: false);
		}
	}
	...
	private void FixedUpdate()
	{
		if (!ps.jumpRope)
		{
			...
			if (Physics.Raycast(base.transform.position, direction, out var hitInfo, float.PositiveInfinity, 769, QueryTriggerInteraction.Ignore) & (hitInfo.transform.tag == "Player") & ((base.transform.position - player.position).magnitude <= 50f) & (playCool <= 0f))
			{
				playerSeen = true;
				TargetPlayer();
			}
