...
public class MouseAppearingScript : MonoBehaviour
{
	...
	private void Update()
	{
		...
		if (Physics.Raycast(ray, out var hitInfo) && ((hitInfo.collider.tag == "Door") & (Vector3.Distance(playerTransform.position, hitInfo.transform.position) <= 12f)))
		{
		...
		else if (Physics.Raycast(ray, out hitInfo) && ((hitInfo.collider.name == "SleepingPrize") & (Vector3.Distance(playerTransform.position, hitInfo.transform.position) <= 10f)))
		{
			MouseCursor.SetActive(value: true);
		}
		else
