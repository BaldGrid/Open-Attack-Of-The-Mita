...
public class PrincipalScript : MonoBehaviour
{
	...
	private int[] lockTime = new int[6] { 20, 25, 35, 40, 50, 55 };
	...
	private void FixedUpdate()
	{
		if (!angry)
		{
			...
			if (Physics.Raycast(base.transform.position + Vector3.up * 2f, aim, out hit, float.PositiveInfinity, 769, QueryTriggerInteraction.Ignore) & (hit.transform.tag == "Player") & (playerScript.guilt > 0f) & !inOffice & !angry)
			{
				seesRuleBreak = true;
			}
			else
			...
			if (Physics.Raycast(base.transform.position + Vector3.up * 2f, aim, out hit, float.PositiveInfinity, 769) & (hit.transform.name == "Its a Bully") & (bullyScript.guilt > 0f) & !inOffice & !angry)
			{
				TargetBully();
			}
		}
	...
	private void OnTriggerStay(Collider other)
	{
		...
		if ((other.tag == "Player") & angry)
		{
			...
			agent.Warp(new Vector3(10f, 0f, 175f));
